...
) => CborType<T | null, T | null, EE, DE | DecodingError, EC, DC> {
  return <T, EE extends Error, DE extends Error, EC, DC>(
    ty: ICborTypeCodec<T, T, EE, DE, EC, DC>
  ): CborType<T | null, T | null, EE, DE | DecodingError, EC, DC> =>
    CborType.builder()
      .encode((value: T | null, e: IEncoder, ctx: any): Result<void, EE> => {
        if (value == null) {
          e.write(NULL_BYTE);
          return getVoidOk();
        }
      ...
      .decode((d: IDecoder, ctx: any): Result<T | null, DE | DecodingError> => {
